...
function updateSymbol(e) {
  var selected = document.querySelector(".currency-selector option:checked");
  ...
  var currencyAddonsFixed = document.querySelectorAll(".currency-addon-fixed");

  for (var i = 0; i < currencySymbols.length; i++) {
      currencySymbols[i].textContent = selected.getAttribute("data-symbol");
  }

  for (var j = 0; j < currencyAmounts.length; j++) {
      currencyAmounts[j].placeholder = selected.getAttribute("data-placeholder");
  }

  for (var k = 0; k < currencyAddonsFixed.length; k++) {
      currencyAddonsFixed[k].textContent = selected.textContent;
  }
}
